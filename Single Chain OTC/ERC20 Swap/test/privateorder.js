...
describe("OTC Desk", function () {
    async function deployTokenFixture() {
        ...
        const Taker = taker.address;
        const tokenToBuy = token2contract.address;
        const buyAmount = 100;
        const sig = "0x8e08acde6bf1d3c950a8936e98a38d8d2fbd5ded591fa8398cdc35490951306c59a1f5c33160338a53517df43288f4dd1c6fb4027f642790d9ab6318870544671b"
        ...
        return { owner, maker, taker, swapcontract, token1contract, token2contract, nonce, Maker, Taker, tokenToSell, sellAmount, tokenToBuy, buyAmount, signature };
    }

    it("Contract Paused", async function () {
        const { owner, maker, taker, swapcontract, token1contract, token2contract, nonce, Maker, tokenToSell, Taker, sellAmount, tokenToBuy, buyAmount, signature } = await loadFixture(deployTokenFixture);
        await swapcontract.connect(owner).pause();
        await expect(swapcontract.connect(taker).swapPrivateOrder(nonce, 1, Maker, tokenToSell, sellAmount, Taker, tokenToBuy, buyAmount, signature)).to.be.reverted;
    ...
    it("OTC Swap without platform fees", async function () {
        const { owner, maker, taker, swapcontract, token1contract, token2contract, nonce, Maker, Taker, tokenToSell, sellAmount, tokenToBuy, buyAmount, signature } = await loadFixture(deployTokenFixture);
        ...
        await swapcontract.connect(taker).swapPrivateOrder(nonce, 1, Maker, tokenToSell, sellAmount, Taker, tokenToBuy, buyAmount, signature);
    ...
    it("OTC Swap with platform fees", async function () {
        const { owner, maker, taker, swapcontract, token1contract, token2contract, nonce, Maker, tokenToSell, sellAmount, Taker, tokenToBuy, buyAmount, signature } = await loadFixture(deployTokenFixture);
